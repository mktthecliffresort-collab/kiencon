...
function generateLocalFallback(input: SocraticTutorInput): SocraticTutorOutput {
  ...
  if (isGrade5) {
    // Grade 5 Analogies: food, playground, toys
    if (guidanceLevel === 1) {
      return {
        guidanceLevel: 1,
    ...
    // Grade 8 Analogies: physics phenomena, engineering, sand dunes Mũi Né, tractors
    if (guidanceLevel === 1) {
      return {
        guidanceLevel: 1,
...
/**
 * Evaluates student input and generates Socratic tutoring guidance following strict rules.
 */
export async function getSocraticTutorGuidance(input: SocraticTutorInput): Promise<SocraticTutorOutput> {
  const { studentGrade, subject, currentTopic, questionContext, studentInput, attemptCount } = input;
...
`;
  ...
  for (const modelName of modelsToTry) {
    try {
      const config: any = {
        ...
        responseSchema: {
          ...
          properties: {
            guidanceLevel: {
              type: Type.INTEGER,
              description: "1 (Nudge/Question) | 2 (Analogy/Hint) | 3 (Detailed Explanation)",
            },
            responseMessage: {
              type: Type.STRING,
              description: "Concise Vietnamese response from Chú Kiến matching student age tone.",
            },
            followUpQuestion: {
              type: Type.STRING,
              description: "A question prompting the student to think instead of receiving direct answers.",
            },
            misconceptionDetected: {
              type: Type.STRING,
              description: "Optional explanation if the student holds a common misconception, or empty string if none.",
            },
          },
          required: ["guidanceLevel", "responseMessage", "followUpQuestion"],
        },
      };
      ...
      // Enforce guidanceLevel constraints
      let level = parsed.guidanceLevel || (attempts <= 1 ? 1 : attempts === 2 ? 2 : 3);
      ...
      lastError = err;
      console.warn(`Model ${modelName} failed or unavailable, attempting next in fallback chain...`);
    }
  }

  console.warn("All Gemini Socratic Tutor models busy/failed, using intelligent local fallback:", lastError?.message || lastError);
  return generateLocalFallback(input);
}
